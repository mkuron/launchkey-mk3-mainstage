...
controls = {
	{name='Keyboard', label=DEVICE_NAME, objectType='Keyboard', midiType='Keyboard', startKey=48, numberKeys=25, midi={0x90,MIDI_Wildcard,MIDI_Wildcard}},
...
function controller_initialize(appName, deviceNewlyDetected)
	return {
		midi = {
			...
			0xf0, 0x00, 0x20, 0x29, 0x02, DEVICE_PID, 0x04, 0, string.crunch(appName, 16), 0xf7,
			0xf0, 0x00, 0x20, 0x29, 0x02, DEVICE_PID, 0x04, 1, " ", 0xf7,
		},
		outport = DAW_IN
	}
end
...
function controller_select_patch(programchangeNumber, patchname, setname, concertname, patchlist, currentSetIndex, currentPatchIndex)
	...
	event = {
		...
		0xF0, 0x00, 0x20, 0x29, 0x02, DEVICE_PID, 0x04, 1, string.crunch(patchname, 16), 0xF7,
	}
	if HAS_FADERS then
	...
	if midiEvent[0] == 0xBF then
		if midiEvent[1] >= 0x15 and midiEvent[1] <= 0x1C then
			...
			if name == labelDisplayCache[midiEvent[1]] then
				event = {
					0xF0, 0x00, 0x20, 0x29, 0x02, DEVICE_PID, 0x08, midiEvent[1]-0x15+0x38, string.crunch(val, 16), 0xF7
				}
			else
				event = {
					0xF0, 0x00, 0x20, 0x29, 0x02, DEVICE_PID, 0x07, midiEvent[1]-0x15+0x38, string.crunch(name, 16), 0xF7,
					0xF0, 0x00, 0x20, 0x29, 0x02, DEVICE_PID, 0x08, midiEvent[1]-0x15+0x38, string.crunch(val, 16), 0xF7
				}
		...
		elseif HAS_FADERS and midiEvent[1] >= 0x35 and midiEvent[1] <= 0x3D then
			...
			if name == labelDisplayCache[midiEvent[1]] then
				event = {
					0xF0, 0x00, 0x20, 0x29, 0x02, DEVICE_PID, 0x08, midiEvent[1]-0x35+0x50, string.crunch(val, 16), 0xF7
				}
			else
				event = {
					0xF0, 0x00, 0x20, 0x29, 0x02, DEVICE_PID, 0x07, midiEvent[1]-0x35+0x50, string.crunch(name, 16), 0xF7,
					0xF0, 0x00, 0x20, 0x29, 0x02, DEVICE_PID, 0x08, midiEvent[1]-0x35+0x50, string.crunch(val, 16), 0xF7
				}
				labelDisplayCache[midiEvent[1]] = name
			end
